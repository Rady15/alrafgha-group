...
const HomePage = () => {
  ...
  const stats = [
    { value: '1000+', label: t('stats.happyRiders'), icon: Users },
    { value: '15+', label: t('stats.vehicles'), icon: Car },
    { value: '3+', label: t('stats.citiesLive'), icon: MapPin },
    { value: '4.9', label: t('stats.starRating'), icon: Star },
  ];
  ...
  const testimonials = [
    { name: 'محمد العتيبي', role: t('testimonials.weekendRoadTripper'), text: t('testimonials.comment1', { amount: '200 ر.س' }), rating: 5 },
    { name: 'نورة الدوسري', role: t('testimonials.cityCommuter'), text: t('testimonials.comment2'), rating: 5 },
    { name: 'خالد القحطاني', role: t('testimonials.frequentTraveler'), text: t('testimonials.comment3'), rating: 5 },
  ];

  const filterTabs = [['all', t('categories.badge')], ['car', t('categories.cityCars')], ['bike', t('categories.commuterBikes')]];
  ...
  return (
    <div className="alrafgha-home" dir="rtl">
      ...
      <section className="home-hero" data-testid="hero-section">
        ...
        <div className="home-shell home-hero__content">
          <div className="home-hero__copy">
            <span className="home-kicker"><span className="home-kicker__dot" /> {t('hero.subtext').split('.')[0]}.</span>
            <h1>{t('hero.headline1')} <span>{t('hero.headlineTap')}</span> {t('hero.headline2')}</h1>
            <p>{t('hero.subtext')}</p>
          ...
          <div className="home-hero__visual">
            <img src="https://images.unsplash.com/photo-1503376780353-7e6692767b70?w=1500&h=1000&fit=crop" alt={t('hero.allVehicles')} />
            ...
            <div className="home-hero__floating home-hero__floating--bottom">
              <span className="home-live-dot" /> {t('hero.pillSuffix')}
            </div>
          </div>
        </div>
        ...
        <div className="home-search-wrap home-shell">
          <form className="home-search" onSubmit={submitSearch}>
            <div className="home-search__field home-search__field--location">
              <MapPin size={19} />
              <label>{t('howItWorks.book.title')}<input value={location} onChange={(e) => setLocation(e.target.value)} placeholder={t('howItWorks.book.description').split('.')[0]} /></label>
            </div>
            <div className="home-search__field">
              <CalendarDays size={19} />
              <label>{t('howItWorks.book.title')}<input type="date" value={pickupDate} onChange={(e) => setPickupDate(e.target.value)} /></label>
            </div>
            <div className="home-search__field">
              <CalendarDays size={19} />
              <label>{t('howItWorks.book.title')}<input type="date" value={returnDate} onChange={(e) => setReturnDate(e.target.value)} /></label>
            </div>
            <button className="home-search__submit" type="submit"><Search size={19} /> {t('hero.allVehicles')}</button>
          </form>
        </div>
      ...
      <section className="home-section home-section--soft" data-testid="categories-section">
        <div className="home-shell">
          <div className="home-section-head">
            <div><span className="home-overline">{t('categories.badge')}</span><h2>{t('categories.pickYour')} <span>{t('categories.vibe')}</span></h2></div>
            <p>{t('categories.subtitle')}</p>
          </div>
          <div className="home-category-grid">
            {categoryCards.map((category) => (
              <Link key={category.key} to={`/vehicles?type=${category.type}`} className="home-category-card">
                <img src={category.image} alt={t(`categories.${category.key}`)} />
                <div className="home-category-card__overlay" />
                <div className="home-category-card__top"><span>{category.type === 'car' ? <Car size={13} /> : <Bike size={13} />}</span><span>{categoryCounts[category.key]} {t('stats.vehicles')}</span></div>
                <div className="home-category-card__bottom"><h3>{t(`categories.${category.key}`)}</h3><span>{t('categories.options', { count: categoryCounts[category.key] })}</span></div>
              </Link>
            ))}
          </div>
        </div>
      ...
      <section className="home-section" data-testid="featured-vehicles-section">
        <div className="home-shell">
          <div className="home-section-head home-section-head--compact">
            <div><span className="home-overline">{t('featured.editorsPick')}</span><h2>{t('featured.featuredRides')}</h2></div>
            <Link to="/vehicles" className="home-text-link">{t('featured.viewAll')} <ArrowLeft size={16} /></Link>
          </div>
          <div className="home-filter-pills" role="tablist" aria-label={t('categories.badge')}>
      ...
      <section className="home-owner-cta">
        <div className="home-owner-cta__image"><img src="https://images.unsplash.com/photo-1492144534655-ae79c964c9d7?w=1300&h=800&fit=crop" alt={t('cta.browseVehicles')} /></div>
        <div className="home-owner-cta__content home-shell">
          <div><span className="home-overline home-overline--light">{t('cta.specialOffer')}</span><h2>{t('cta.readyToHit')} <span>{t('cta.theRoad')}</span></h2><p>{t('cta.subtitle')}</p></div>
          <Link to="/vendor" className="home-btn home-btn--primary">{t('cta.browseVehicles')} <ArrowUpLeft size={17} /></Link>
        </div>
      </section>
      ...
      <section className="home-section home-section--soft" data-testid="features-section">
        <div className="home-shell">
          <div className="home-section-head home-section-head--center"><div><span className="home-overline">{t('features.badge')}</span><h2>{t('features.title1')} <span>{t('features.freedom')}</span> {t('features.title2')} <span>{t('features.honesty')}</span></h2></div><p>{t('features.subtitle')}</p></div>
      ...
      <section className="home-process" data-testid="how-it-works-section">
        <div className="home-shell">
          <div className="home-section-head home-section-head--light"><div><span className="home-overline home-overline--light">{t('howItWorks.badge')}</span><h2>{t('howItWorks.title')} <span>{t('howItWorks.openRoad')}</span></h2></div><p>{t('howItWorks.discover.description')}</p></div>
      ...
      <section className="home-section" data-testid="experience-section">
        <div className="home-shell home-experience">
          <div className="home-experience__photo"><img src="https://images.unsplash.com/photo-1542296332-2e4473faf563?w=1000&h=1200&fit=crop" alt={t('experience.premiumBadge')} /><div className="home-experience__badge"><Check size={15} /> {t('experience.premiumBadge')}</div></div>
          <div className="home-experience__copy"><span className="home-overline">{t('experience.title1')}</span><h2>{t('experience.titleEm')}</h2><p>{t('experience.subtitle')}</p><div className="home-experience__list"><div><Clock3 size={19} /><span><strong>{t('experience.item1Title')}</strong> {t('experience.item1Desc')}</span></div><div><ShieldCheck size={19} /><span><strong>{t('experience.item2Title')}</strong> {t('experience.item2Desc')}</span></div><div><Headset size={19} /><span><strong>{t('experience.item3Title')}</strong> {t('experience.item3Desc')}</span></div></div><Link to="/vehicles" className="home-btn home-btn--primary">{t('experience.startJourney')} <ArrowLeft size={17} /></Link></div>
        </div>
      </section>
      ...
      {blogPosts.length > 0 && (
        <section className="home-section home-section--soft">
          <div className="home-shell">
            <div className="home-section-head home-section-head--compact"><div><span className="home-overline">{tCommon('nav.blog')}</span><h2>{t('featured.featuredRides')}</h2></div><Link to="/blog" className="home-text-link">{t('featured.viewAll')} <ArrowLeft size={16} /></Link></div>
            <div className="home-articles">{blogPosts.map((post) => <Link to={`/blog/${post.slug}`} className="home-article" key={post._id}><div className="home-article__image"><img src={post.featured_image || 'https://images.unsplash.com/photo-1593941707882-a5bba14938c7?w=900&h=600&fit=crop'} alt={post.title} /><span>{post.category}</span></div><div className="home-article__body"><h3>{post.title}</h3><p>{post.excerpt || t('features.subtitle')}</p><span>{t('experience.startJourney')} <ArrowLeft size={15} /></span></div></Link>)}</div>
          </div>
        </section>
      )}
      ...
      <section className="home-section" data-testid="testimonials-section">
        <div className="home-shell">
          <div className="home-section-head home-section-head--center"><div><span className="home-overline">{t('testimonials.badge')}</span><h2>{t('testimonials.title')} <span>{t('testimonials.titleEm')}</span></h2></div></div>
      ...
      <section className="home-app-cta">
        <div className="home-shell home-app-cta__inner"><div className="home-app-cta__phone"><div className="home-phone-notch" /><img src="/arafgha-logo.png" alt="Alrafgha Group" /></div><div><span className="home-overline home-overline--light">{t('cta.specialOffer')}</span><h2>{t('cta.readyToHit')} <span>{t('cta.theRoad')}</span></h2><p>{t('cta.subtitle')}</p><div className="home-app-buttons"><span><small>App Store</small><strong>App Store</strong></span><span><small>Google Play</small><strong>Google Play</strong></span></div></div></div>
      </section>

      <section className="home-newsletter">
        <div className="home-shell home-newsletter__inner"><div><span className="home-overline">{t('stats.trustedBy')}</span><h2>{t('cta.readyToHit')} <span>{t('cta.theRoad')}</span></h2></div><form onSubmit={(e) => e.preventDefault()}><input type="email" placeholder={t('hero.subtext').split('.')[0]} aria-label={t('hero.subtext').split('.')[0]} /><button type="submit">{tCommon('actions.next')} <ArrowLeft size={15} /></button></form></div>
      </section>
    </div>
  );
};
