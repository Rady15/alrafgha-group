      ...
      try {
        ...
        if (result.success) {
          ...
          if (result.requiresVerification && result.email) {
            toast.error(result.message || t('auth:login.toastVerifyEmail'));
            navigate('/verify-otp', { state: { email: result.email, userType: result.userType || 'user' } });
          } else {
            toast.error(result.message || t('auth:login.toastFailed'));
          }
        }
      } catch (error) {
        setLoading(false);
        toast.error(t('auth:login.toastError', { message: error.message }));
      }
    }
  };
  ...
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        setLoading(false);
        if (result.success) {
          toast.success(t('auth:login.toastGoogleSuccess'));
          ...
          if (result.requiresVerification && result.email) {
            ...
            toast.error(result.message || t('auth:login.toastGoogleFailed'));
          }
        }
      } catch (error) {
        setLoading(false);
        toast.error(t('auth:login.toastGoogleError', { message: error.message }));
      }
    },
    onError: () => toast.error(t('auth:login.toastGoogleFailedSimple')),
  });
      ...
      <div className="hidden lg:flex w-1/2 relative overflow-hidden bg-ink-950 text-white p-12 flex-col justify-between">
        ...
        <div className="relative z-10 max-w-md">
          ...
          <h1 className="font-display font-bold text-5xl xl:text-7xl leading-[0.9] tracking-[-0.04em] mb-5">
            ...
            <br />
            <em className="not-italic text-gradient-sunset">{t('auth:login.heroLine2')}</em>
          </h1>
          ...
          <div className="mt-10 grid grid-cols-3 gap-3 max-w-md">
            {[
              { v: '1000+', l: t('auth:login.statRiders') },
              { v: '4.9', l: t('auth:login.statRating') },
              { v: '24/7', l: t('auth:login.statSupport') },
            ].map((s, i) => (
        ...
        <div className="relative z-10 flex items-center gap-4 text-xs text-ink-300">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span>{t('auth:login.lovedByTravelers')}</span>
          </div>
        </div>
      </div>
        ...
        <div className="w-full max-w-md space-y-7 animate-fade-in-up relative">
          ...
          <div>
            ...
            <h2 className="font-display font-bold text-4xl sm:text-5xl text-ink-900 leading-[0.95] tracking-tight">
              {t('auth:login.heading')}<span className="text-primary-500">.</span>
            </h2>
            <p className="text-ink-500 mt-2 text-base">{t('auth:login.subtitle')}</p>
          </div>
