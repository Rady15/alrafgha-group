...
const VerifyOtpPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (otpString.length !== 6) {
            toast.error(t('auth:verifyOtp.toastInvalidOtp'));
            return;
        }
        ...
        try {
            ...
            if (result.success) {
                if (userType === 'vendor') {
                    toast.success(result.message || t('auth:verifyOtp.toastVendorVerified'));
                    navigate('/login');
                } else {
                    toast.success(t('auth:verifyOtp.toastVerified'));
                    navigate('/');
                }
            } else {
                toast.error(result.message || t('auth:verifyOtp.toastFailed'));
                // Clear OTP on error
                setOtp(['', '', '', '', '', '']);
                inputRefs.current[0]?.focus();
            }
        } catch (error) {
            toast.error(t('auth:verifyOtp.toastError', { message: error.message }));
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleResend = async () => {
        ...
        try {
            ...
            if (result.success) {
                toast.success(t('auth:verifyOtp.toastResent'));
                setCountdown(60);
                setCanResend(false);
                setOtp(['', '', '', '', '', '']);
                inputRefs.current[0]?.focus();
            } else {
                toast.error(result.message || t('auth:verifyOtp.toastResendFailed'));
            }
        } catch (error) {
            toast.error(t('auth:verifyOtp.toastResendError', { message: error.message }));
        } finally {
    ...
    const getBackLinkText = () => {
        return userType === 'vendor' ? t('auth:verifyOtp.backLinkVendor') : t('auth:verifyOtp.backLinkUser');
    };
                ...
                <div className="bg-white rounded-2xl shadow-2xl p-8 space-y-6 animate-fade-in">
                    ...
                    <div className="text-center">
                        ...
                        <h2 className="text-3xl font-display font-bold text-neutral-900 mb-2">
                            {t('auth:verifyOtp.headingPre')} <span className='text-red-500'>{t('auth:verifyOtp.email')}</span>
                        </h2>
                        {userType === 'vendor' && (
                            <span className="inline-block px-3 py-1 bg-primary-100 text-primary-700 rounded-full text-xs font-semibold mb-2">
                                {t('auth:verifyOtp.vendorRegistration')}
                            </span>
                        )}
                        <p className="text-neutral-600 text-sm">
                            {t('auth:verifyOtp.sentOtpTo')}
                        </p>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="text-center">
                            {canResend ? (
                                ...
                                >
                                    {resendLoading ? t('auth:shared.sending') : t('auth:shared.resendOtp')}
                                </button>
                            ) : (
                                <p className="text-neutral-500 text-sm">
                                    {t('auth:verifyOtp.resendInPrefix')} <span className="font-semibold text-primary-600">{countdown}s</span>
                                </p>
                            )}
                        </div>
                        ...
                        >
                            {loading ? (
                                ...
                                <span>{t('auth:verifyOtp.verifyEmail')}</span>
                            )}
                        </button>
                    </form>
                            ...
                            <div className="flex items-start space-x-3">
                                ...
                                <div>
                                     <p className="text-sm text-amber-800">
                                         <span className="font-semibold">{t('auth:verifyOtp.vendorNoteTitle')}</span> {t('auth:verifyOtp.vendorNoteBody')}
                                     </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex items-start space-x-3">
                            ...
                            <div>
                                     <p className="text-sm text-blue-800">
                                         <span className="font-semibold">{t('auth:verifyOtp.didntReceive')}</span>
                                     </p>
                                     <p className="text-xs text-blue-700 mt-1">
                                     {t('auth:verifyOtp.checkSpam')}
                                 </p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="text-center mt-6">
                    <Link to="/" className="text-white hover:text-white/80 transition-colors duration-200 text-sm font-medium flex items-center justify-center space-x-1">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                         ...
                         <span>{t('auth:verifyOtp.backToHome')}</span>
                    </Link>
                </div>
            </div>
        </div>
