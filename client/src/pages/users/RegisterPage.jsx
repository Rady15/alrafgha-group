...
const RegisterPage = () => {
  ...
  const strengthLabels = [t('auth:register.strength.weak'), t('auth:register.strength.fair'), t('auth:register.strength.good'), t('auth:register.strength.strong'), t('auth:register.strength.excellent')];
  ...
  const validateForm = () => {
    ...
    else if (!/^\+966[0-9]{9}$/.test(formData.phone)) newErrors.phone = 'Phone number must be 9 digits';
      ...
      try {
        ...
        if (result.success) {
          if (result.requiresVerification || result.data?.requiresVerification) {
            toast.success(result.message || t('auth:register.toastVerify'));
          ...
          toast.error(result.message || t('auth:register.toastFailed'));
        }
      } catch (error) {
        setLoading(false);
        toast.error(t('auth:register.toastError', { message: error.message }));
      }
    }
  };
  ...
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        setLoading(false);
        if (result.success) {
          toast.success(t('auth:register.toastGoogleSuccess'));
          navigate('/');
        } else {
          toast.error(result.message || t('auth:register.toastGoogleFailed'));
        }
      } catch (error) {
        setLoading(false);
        toast.error(t('auth:register.toastGoogleError', { message: error.message }));
      }
    },
    onError: () => toast.error(t('auth:register.toastGoogleFailedSimple')),
  });
  ...
  const benefits = [
    { icon: Zap, text: t('auth:register.benefit1') },
    { icon: ShieldCheck, text: t('auth:register.benefit2') },
    { icon: Sparkles, text: t('auth:register.benefit3') },
    { icon: Headset, text: t('auth:register.benefit4') },
  ];
    ...
    <div className="min-h-screen flex bg-[#fafaf7]" data-testid="register-page">
      ...
      <div className="hidden lg:flex w-1/2 relative overflow-hidden bg-ink-950 text-white p-12 flex-col justify-between">
          ...
          <Link to="/" className="flex items-center gap-3 group">
            ...
            <div className="flex flex-col leading-none">
              ...
              <span className="text-[10px] font-display tracking-[0.1em] text-ink-400 font-medium mt-0.4">
                 <span className='text-primary-500 font-bold'>~</span> {t('common:brandTagline')}
              </span>
            </div>
          </Link>
        ...
        <div className="relative z-10 text-xs text-ink-400">
           {t('auth:register.copyright', { year: new Date().getFullYear() })}
        </div>
      </div>
        ...
        <div className="w-full max-w-md space-y-6 animate-fade-in-up relative py-6">
          ...
          <div>
            ...
            <h2 className="font-display font-bold text-4xl sm:text-5xl text-ink-900 leading-[0.95] tracking-tight">
              {t('auth:register.headingLead')} <em className="not-italic text-gradient-warm">{t('auth:register.headingEmphasis')}</em>
            </h2>
            <p className="text-ink-500 mt-2 text-base">{t('auth:register.subtitle')}</p>
          </div>
          ...
          >
            ...
            <span className="text-sm font-bold text-ink-900">{t('auth:shared.continueWithGoogle')}</span>
          </button>
