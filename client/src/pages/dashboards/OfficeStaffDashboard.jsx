...
const OfficeStaffDashboard = () => {
    ...
    const handleMarkRefundReturned = async (booking) => {
        if (!window.confirm(t('refund.confirmMarkReturned', { bill: booking.bill_id || booking._id }))) {
            return;
        }
    ...
    const getStatusBadge = (status) => {
        ...
        const labels = {
            booking_requested: t('status.pending'),
            picked_up: t('status.active'),
            returned: t('status.completed'),
                ...
                <div className="mb-8 md:mb-12">
                    ...
                    <div className="flex flex-col md:flex-row md:justify-between md:items-center space-y-4 md:space-y-0">
                        ...
                        <div className='text-center md:flex-1'>
                            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900">{t('title.officeStaff')} <span className='text-red-600'>{t('title.dashboard')}</span></h1>
                            <p className="mt-1 md:mt-2 text-sm md:text-base text-gray-600">{t('subtitle.officeStaff')}</p>
                        </div>
                ...
                <div className="flex flex-col md:flex-row gap-6">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        {loading ? (
                            ...
                            <h3 className="mt-2 text-sm font-medium text-gray-900">{t('empty.noBookings')}</h3>
                            <p className="mt-1 text-sm text-gray-500">
                                {activeTab === 'pending' && t('empty.noPending')}
                                {activeTab === 'active' && t('empty.noActive')}
                                {activeTab === 'completed' && t('empty.noCompleted')}
                                {activeTab === 'cancelled' && t('empty.noCancelled')}
                            </p>
                            </div>
                        ) : (
                                            ...
                                            {activeTab === 'completed' ? (
                                                <>
                                                    ...
                                                    >
                                                        <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-3 space-y-2 sm:space-y-0">
                                                            <h3 className="text-base md:text-lg font-semibold text-gray-900">
                                                                {booking.vehicle_id?.name || t('na')} - {booking.vehicle_id?.model_name || t('na')}
                                                            </h3>
                                                            
                                                        </div>
                                                        <button 
                                                            className="p-2 hover:bg-gray-100 rounded-full transition-colors"
                                                            aria-label={expandedCards[booking._id] ? t('a11y.collapseDetails') : t('a11y.expandDetails')}
                                                        >
                                                    ...
                                                    {!expandedCards[booking._id] && (
                                                        <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-sm text-gray-600">
                                                            <span>{t('details.customer')}: <span className="font-medium text-gray-900">{booking.user_id?.name || t('na')}</span></span>
                                                            <span>•</span>
                                                            <span>{t('details.returned')}: <span className="font-medium text-gray-900">{formatDateDDMMYYYY(booking.return_details?.actual_return_date)}</span></span>
                                                            {booking.bill_id && (
                                                                <>
                                                                    <span>•</span>
                                                        ...
                                                        <div className="border-t border-gray-100 pt-4">
                                                            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
                                                                <div>
                                                                    ...
                                                                    <p className="font-medium text-gray-900">{booking.user_id?.name || t('na')}</p>
                                                                    <p className="text-gray-600 wrap-break-words">{booking.user_id?.email || t('na')}</p>
                                                                    <p className="text-gray-600">{booking.user_id?.phone || t('na')}</p>
                                                                </div>

                                                                <div>
                                                                    <p className="text-gray-500">{t('details.vehicleDetails')}</p>
                                                                    <p className="font-medium text-gray-900">{booking.vehicle_id?.registration_number || t('na')}</p>
                                                                    <p className="text-gray-600">{booking.vehicle_id?.type || t('na')} - {booking.vehicle_id?.cc_engine || t('na')}cc</p>
                                                                </div>

                                                                <div>
                                                                    <p className="text-gray-500">{t('details.pickupLocation')}</p>
                                                                ...
                                                                <div>
                                                                    <p className="text-gray-500">{t('details.vehiclePickup')}</p>
                                                                ...
                                                                {booking.return_details && (
                                                                    <>
                                                                        <div>
                                                                            <p className="text-gray-500">{t('details.vehicleReturned')}</p>
                                                                        ...
                                                                        {booking.pickup_details && (
                                                                            <div>
                                                                                <p className="text-gray-500">{t('details.customerIdDetails')}</p>
                                                                                <p className="text-gray-600">{t('details.govtId')}: <span className="text-gray-900">{booking.pickup_details.id_proof_type?.replace('_', ' ').toUpperCase()}</span></p>
                                                                                <p className="text-gray-600">{t('details.idNumber')}: {booking.pickup_details.id_number || t('na')}</p>
                                                                            </div>
                                                                        )}
                                                                    </>
                                                                )}
                                                                ...
                                                                {booking.return_details?.staff_id && (
                                                                    <div>
                                                                        <p className="text-gray-500">{t('details.returnStaff')}</p>
                                                                ...
                                                                {booking.bill_id && (
                                                                    <div>
                                                                        <p className="text-gray-500">{t('details.billId')}</p>
                                                ...
                                                <div className="flex flex-col space-y-4">
                                                    ...
                                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
                                                        ...
                                                        <div>
                                                            <p className="text-gray-500">{t('details.requestedPickup')}</p>
                                                            <p className="font-medium text-gray-900">
                                                        ...
                                                        <div>
                                                            <p className="text-gray-500">{t('details.cancelledBy')}</p>
                                                            <p className="font-medium text-red-900 capitalize">                                                {booking.cancelled_by === 'staff' ? t('party.staff') : t('party.customer')}</p>
                                                            {booking.cancelled_at && (
                                                                <p className="text-gray-600 text-xs">                                                    {formatDateTime(booking.cancelled_at)}</p>
                                                            )}
                                                        </div>
                                                        ...
                                                        {booking.refund_amount > 0 && (
                                                            <div>
                                                                ...
                                                                <p className={`text-xs font-semibold ${
                                                                    ...
                                                                    {booking.refund_status === 'completed' ? `✅ ${t('refund.returned')}` : `⏳ ${t('refund.pending')}`}
                                                                </p>
                                                            </div>
                                                        )}
                                                    </div>
                                                    ...
                                                    {(booking.cancellation_reason || booking.rejection_reason) && (
                                                        <div className="mt-3 p-3 bg-red-50 border border-red-200 rounded-lg">
                                                            <p className="text-sm font-semibold text-red-900 mb-1">
                                                                {booking.cancelled_by === 'staff' ? t('reason.rejection') : t('reason.cancellation')}
                                                            </p>
                                                    ...
                                                    {booking.refund_status === 'pending' && booking.refund_amount > 0 && (
                                                        <div className="pt-2">
                                                            ...
                                                            >
                                                                {markingRefund ? t('refund.processing') : t('refund.markReturned')}
                                                            </button>
                                                        </div>
                                                    )}
                                                </div>
                                                ...
                                                <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between space-y-4 lg:space-y-0">
                                                    <div className="flex-1">
                                                        ...
                                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
                                                            ...
                                                            {booking.pickup_details && (
                                                                <div>
                                                                    <p className="text-gray-500">{t('details.pickupInfo')}</p>
                                                                    <p className="font-medium text-gray-900">
                                                                        {t('details.odometer', { value: booking.pickup_details.odometer_reading_start })} km
                                                                    </p>
                                                                    <p className="text-gray-600">{t('details.govtId')}: {booking.pickup_details.id_proof_type?.replace('_', ' ').toUpperCase()}</p>
                                                                    <p className="text-gray-600">{t('details.idNumber')}: {booking.pickup_details.id_number || t('na')}</p>
                                                                </div>
                                                            )}
            ...
            {showRejectDialog && selectedBooking && (
                <div className="fixed inset-0 backdrop-blur-md bg-opacity-50 flex items-center justify-center z-150 p-4">
                    <div className="bg-white border-2 border-primary-200 rounded-lg shadow-xl max-w-md w-full p-6">
                        <h3 className="text-xl font-bold text-red-600 mb-4">{t('actions.rejectBooking')}</h3>

                        <div className="mb-4">
                                <p className="text-sm text-gray-600 mb-2">
                                    {t('details.customer')}: <span className="font-medium text-gray-900">{selectedBooking.user_id?.name || t('na')}</span>
                                </p>
                                <p className="text-sm text-gray-600 mb-4">
                                    {t('details.vehicle')}: <span className="font-medium text-gray-900">{selectedBooking.vehicle_id?.name || t('na')}</span>
                                </p>
                        </div>

                        <div className="mb-6">
                            <label htmlFor="rejection-reason" className="block text-sm font-medium text-gray-700 mb-2">
                                        {t('reason.rejection')} <span className='text-red-500'>*</span>
                            </label>
                            <textarea
                                id="rejection-reason"
