...
const VendorPage = () => {
  ...
  const handleChange = (e) => {
    ...
    // Handle contact_number with automatic +966 prefix
    if (name === 'contact_number') {
      // Remove any non-digit characters
      const digitsOnly = value.replace(/[^\d]/g, '');
      if (digitsOnly.length <= 10) {
        finalValue = digitsOnly ? `+966${digitsOnly}` : '';
      } else {
        return;
      }
    }
  ...
  const handleFileChange = async (e) => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      toast.warning(t('toast.fileSize'));
      return;
    }
    ...
    try {
      ...
      if (data.status === 'success') {
        ...
        toast.success(t('toast.docSuccess'));
      } else {
        toast.error(t('toast.docError'));
      }
    } catch (error) {
      toast.error(t('toast.docErrorMsg', { msg: error.message }));
    } finally {
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!formData.name) newErrors.name = t('form.nameRequired');
    if (formData.is_organization && !formData.company_name) {
      newErrors.company_name = t('form.companyNameRequired');
    }
    if (!formData.contact_number) {
      newErrors.contact_number = t('form.contactRequired');
    } else if (!/^\+966[0-9]{9}$/.test(formData.contact_number)) {
      newErrors.contact_number = t('form.contactDigits');
    }
    if (!formData.email) newErrors.email = t('form.emailRequired');
    else if (!/\S+@\S+\.\S+/.test(formData.email)) newErrors.email = t('form.emailInvalid');
    if (!formData.id_type) newErrors.id_type = t('form.idTypeRequired');
    if (!documentUrl) newErrors.document = t('form.documentRequired');
    if (!formData.address) newErrors.address = t('form.addressRequired');
    if (!formData.password) newErrors.password = t('form.passwordRequired');
    else if (formData.password.length < 6) newErrors.password = t('form.passwordLength');
    if (formData.password !== formData.confirm_password) {
      newErrors.confirm_password = t('form.passwordMismatch');
    }
    if (!formData.agreeToTerms) {
      newErrors.agreeToTerms = t('form.agreeRequired');
    }
      ...
      try {
        ...
        if (result.success) {
          if (result.requiresVerification) {
            toast.success(result.message || t('toast.registerSuccessOtp'));
            navigate('/verify-otp', {
              state: {
                email: formData.email,
                userType: 'vendor'
          ...
          toast.error(result.message || t('toast.registerError'));
        }
      } catch (error) {
        setLoading(false);
        toast.error(t('toast.registerErrorMsg', { msg: error.message }));
      }
    }
  };
  ...
  const getIdTypeOptions = () => {
    if (formData.is_organization) {
      return [
        { value: '', label: t('form.selectIdType') },
        { value: 'business_reg_certificate', label: t('form.idTypes.businessRegCertificate') },
        { value: 'business_tax_id', label: t('form.idTypes.businessTaxId') },
      ];
    } else {
      return [
        { value: '', label: t('form.selectIdType') },
        { value: 'pan', label: t('form.idTypes.pan') },
        { value: 'license', label: t('form.idTypes.license') },
        { value: 'passport', label: t('form.idTypes.passport') },
        { value: 'adhaar', label: t('form.idTypes.adhaar') },
      ];
    }
  };
      ...
      <section className="relative bg-[#fafaf7] pt-20 pb-32 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <h1 className="font-display font-bold text-5xl sm:text-6xl lg:text-7xl text-ink-900 leading-[0.95] tracking-tight mb-6">
              ...
              <em className="not-italic text-primary-500">{t('hero.titleAccent')}</em>
            </h1>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="font-display font-bold text-4xl sm:text-5xl text-ink-900 leading-[0.95] tracking-tight mb-4">
              {t('benefits.heading')} <em className="not-italic text-primary-500">{t('benefits.headingAccent')}</em>
            </h2>
...
<h3 className="font-display text-xl font-bold text-ink-900 mb-2 leading-tight">{t(`benefits.${benefit.key}`)}</h3>
<p className="text-ink-600 text-sm leading-relaxed">{t(`benefits.${benefit.key}Desc`)}</p>
              </div>
            ))}
          </div>
        </div>
              ...
              <div className="bg-ink-50 rounded-2xl p-4 border border-ink-100 mt-2">
                <label className="flex items-center cursor-pointer group">
                  ...
                  <div className="ml-3 flex items-center gap-2">
                    ...
                    <span className="text-sm font-bold text-ink-900 group-hover:text-primary-600 transition-colors">{t('form.registerAsOrg')}</span>
                  </div>
                </label>
              </div>
                ...
                <div>
                  ...
                  <div className={`relative group rounded-2xl bg-white border-2 transition-all duration-200 ${errors.contact_number ? 'border-secondary-500' : 'border-ink-100 focus-within:border-ink-900'}`}>
                    <div className="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none">
                      ...
                      <span className="ml-1.5 text-ink-700 font-semibold text-sm">+966</span>
                    </div>
                    <input
                      type="tel"
                      name="contact_number"
              ...
              <div>
                ...
                <div className={`relative ${errors.id_type ? 'border-secondary-500' : ''}`}>
                  <CustomDropdown
                    ...
                    placeholder={t('form.selectIdType')}
                  />
                </div>
                    ...
                    <div className="text-center">
                      {uploadingDoc ? (
                        <div className="flex flex-col items-center">
                          ...
                          <p className="text-sm font-semibold text-primary-600">{t('form.uploading')}</p>
                        </div>
                      ) : documentUrl ? (
                        <div className="flex flex-col items-center">
                          ...
                          <p className="text-sm font-semibold text-green-600">{t('form.documentSuccess')}</p>
                          <p className="text-xs text-ink-500 mt-1">{t('form.clickToReplace')}</p>
                        </div>
                      ) : (
                        <div className="flex flex-col items-center">
                          ...
                          <p className="text-sm font-semibold text-ink-900 mb-1">{t('form.clickToUpload')}</p>
                          <p className="text-[10px] text-ink-500 uppercase tracking-wider font-semibold">{t('form.pdfImage')}</p>
                        </div>
                      )}
                    </div>
                  </div>
