...
const BillModal = ({ booking, onClose }) => {
    ...
    const downloadPDF = async () => {
        try {
            ...
            if (!element) {
                toast.error(t('bookings:toast.billNotFound'));
                return;
            }
            ...
            toast.success(t('bookings:toast.billDownloaded'));
        } catch (error) {
            ...
            <div className="bg-white rounded-lg shadow-lg w-full max-w-[900px] max-h-[90vh] overflow-auto">
                ...
                <div className="sticky top-0 bg-white border-b border-gray-200 px-4 py-3 sm:px-6 sm:py-4 md:px-8 md:py-5 flex items-center justify-between z-10">
                    <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-black">
                        {t('bookings:bill.vehicle')} <span className='text-red-500'>{t('bookings:bill.pickupBill')}</span> 
                    </h2>
                    <button
                        onClick={onClose}
                ...
                <div ref={billRef} style={styles.billContainer}>
                    ...
                    <div style={styles.header}>
                        <h1 style={styles.companyName}>{t('bookings:bill.companyName')}</h1>
                        <p style={styles.tagline}>{t('common:brandTagline')}</p>
                    </div>

                    {/* Bill ID and Date */}
                    <div style={styles.row}>
                        <div>
                            <div style={styles.label}>{t('bookings:bill.billId')}</div>
                            <div style={styles.value} data-testid="bill-id">{booking.bill_id}</div>
                        </div>
                        <div style={{ textAlign: 'right' }}>
                            <div style={styles.label}>{t('bookings:bill.date')}</div>
                    ...
                    <div style={styles.section}>
                        <div style={styles.sectionTitle}>{t('bookings:bill.customerDetails')}</div>
                        <div style={styles.detailRow}>
                            <div style={styles.detailLabel}>{t('bookings:bill.name')}</div>
                            <div style={styles.detailValue}>{booking.user_id?.name || 'N/A'}</div>
                        </div>
                        <div style={styles.detailRow}>
                            <div style={styles.detailLabel}>{t('bookings:bill.contactDetails')}</div>
                            <div style={styles.detailValue}>{booking.user_id?.phone || 'N/A'}, {booking.user_id?.email || 'N/A'}</div>
                        </div>
                        <div style={styles.detailRow}>
                            <div style={styles.detailLabel}>{t('bookings:bill.govtIdProof')}</div>
                    ...
                    {/* Vehicle Details */}
                    <div style={styles.section}>
                        <div style={styles.sectionTitle}>{t('bookings:bill.vehicleDetails')}</div>
                        <div style={styles.detailRow}>
                            <div style={styles.detailLabel}>{t('bookings:bill.vehicleLine', { type: booking.vehicle_id?.type || 'N/A' })}</div>
                            <div style={styles.detailValue}>
                                {booking.vehicle_id?.name || 'N/A'} - {booking.vehicle_id?.model_name || 'N/A'}, {booking.vehicle_id?.cc_engine || 'N/A'}cc
                            </div>
                        </div>
                        <div style={styles.detailRow}>
                            <div style={styles.detailLabel}>{t('bookings:bill.registrationNo')}</div>
                    ...
                    {/* Pickup Details */}
                    <div style={styles.section}>
                        <div style={styles.sectionTitle}>{t('bookings:details.pickupDetails')}</div>
                        <div style={styles.detailRow}>
                            <div style={styles.detailLabel}>{t('bookings:bill.pickupDateTimeLine')}</div>
                            <div style={styles.detailValue}>
                                {formatDate(booking.pickup_details?.actual_pickup_date)} at {booking.pickup_details?.actual_pickup_time || 'N/A'}
                            </div>
                        </div>
                        <div style={styles.detailRow}>
                            <div style={styles.detailLabel}>{t('bookings:bill.pickupLocationLine')}</div>
                    ...
                    {booking.pickup_details?.pickup_notes && (
                        <div style={styles.section}>
                            <div style={styles.sectionTitle}>{t('bookings:bill.notes')}</div>
                    ...
                    <div style={styles.footer}>
                        <p style={styles.footerText}>
                            {t('bookings:bill.finalChargesNote')}
                        </p>
                    </div>
                </div>
