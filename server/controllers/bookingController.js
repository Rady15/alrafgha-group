...
exports.createBookingRequest = catchAsync(async (req, res, next) => {
    ...
    // ATOMIC FIX: Use conditional update to prevent race conditions
    // This ensures only ONE concurrent request can book the vehicle
    ...
    if (!vehicle) {
        // Vehicle not found or not available (another booking won the race)
        const exists = await Vehicle.findById(vehicle_id);
        if (!exists) {
            return next(new AppError('Vehicle not found', 404));
        }
        return next(new AppError(`Vehicle is not available (current status: ${exists.availability_status})`, 409));
    }
    ...
    try {
        ...
        if (!packageData) {
            // ROLLBACK: restore vehicle availability since no package found
        ...
        await logAudit({
            ...
            description: `Booking request created for vehicle ${vehicle.name}`,
            after_value: { vehicle_id, start_location, status: 'booking_requested' },
            req
        });
    } catch (error) {
        // ROLLBACK: restore vehicle availability if booking creation fails
        await Vehicle.findByIdAndUpdate(vehicle_id, { $set: { availability_status: 'available' } });
        throw error;
    }
});
...
exports.confirmPickup = catchAsync(async (req, res, next) => {
    ...
    await logAudit({
        ...
        description: `Vehicle picked up for booking ${billId}`,
        before_value: { status: 'booking_requested' },
        after_value: { status: 'picked_up', bill_id: billId },
        req
    });
});
...
exports.confirmReturn = catchAsync(async (req, res, next) => {
    ...
    if (actual_return_date && actual_return_time) {
        // Extract date part only (YYYY-MM-DD) from actual_return_date
        const datePart = actual_return_date.split('T')[0];
        returnDateTime = new Date(`${datePart}T${actual_return_time}:00`);
    ...
    // ATOMIC FIX: Update vehicle status back to available using atomic update
    const updatedVehicle = await Vehicle.findOneAndUpdate(
        { _id: vehicle._id, availability_status: 'booked' },
        {
            $set: { availability_status: 'available' },
            $inc: {
                total_bookings: 1,
                total_distance_traveled: distance_traveled,
                total_hours_booked: duration_hours
            }
        },
        { new: true }
    );
    ...
    if (!updatedVehicle) {
        console.error('WARNING: Vehicle status update failed during return', vehicle._id);
    }

    // Award loyalty points (1 point per 1 SAR spent)
    try {
        const LoyaltyPoint = require('../models/LoyaltyPoint');
        const pointsToAward = Math.floor(final_cost);
        if (pointsToAward > 0) {
            await LoyaltyPoint.create({
                user_id: booking.user_id,
                points: pointsToAward,
                type: 'earned',
                description: `Earned ${pointsToAward} points from booking ${booking.bill_id || booking._id}`,
                booking_id: booking._id,
                expires_at: new Date(Date.now() + 365 * 24 * 60 * 60 * 1000)
            });
        }
    } catch (lpErr) {
        console.error('Loyalty points error:', lpErr);
    }
    ...
    await logAudit({
        ...
        entity_type: 'booking',
        entity_id: booking._id,
        description: `Vehicle returned for booking ${booking.bill_id}`,
        before_value: { status: 'picked_up' },
        after_value: { status: 'returned', final_cost, amount_paid },
        req
    });
});
